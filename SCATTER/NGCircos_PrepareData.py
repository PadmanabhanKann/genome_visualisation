    ...
    if len(sys.argv)==1:
        ...
        if datatype=="SCATTER":
            ...
            for line in fh:
                ...
                if line and line[0] != "#":  # Skip title lines starting with "#"
                    parts = line.split()
                    if len(parts) == 4:
                        chr_val, start_val, end_val, change_val = parts
                        print(f'  {{chr: "{chr_val}", start: "{start_val}", end: "{end_val}", change: "{change_val}", des: "SNP"}}')
            print("]];")
